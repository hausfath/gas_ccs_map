...
from geo_utils import nearest_point_on_ring_km, point_in_polygon
from transport_common import CO2_LIQUEFACTION_USD_PER_T, MODES, NON_PIPELINE_MODES, TransportGraph
...
def transport_from_legs(legs):
    """Delivered transport $/tCO₂ from the chosen route's legs (handling once per merged leg).
    Liquefaction is added once if any non-pipeline mode is used. Returns (transport, liquefaction)."""
    total = 0.0
    for leg in legs:
        m = leg["mode"]
        total += MODES[m]["handling_usd_per_t"] + MODES[m]["usd_per_tkm"] * leg["km"]
...
def pack(transport_usd, liq_usd, mtpa):
    ...
    total = round(transport_usd + liq_usd + storage, 2)
    return {
        "transport_usd": transport_usd,          # haulage only (excl. liquefaction)
        "liquefaction_usd": liq_usd,
        "storage_usd": storage,
        "total_usd": total,                      # haulage + liquefaction + storage
...
def well_route(graph, plant):
    ...
    for tier, conf in (("firm", "firm"), ("draft", "draft"), ("pending", "pending")):
        if res.get(tier):
            ...
            transport, liq = transport_from_legs(legs)
...
def basin_route(graph, plant, basins):
    """In-basin → on-site storage (nominal transport). Else the least-cost multimodal route (existing
    pipelines + truck/rail/barge, NO new pipelines) to the nearest basin edge. Basins are treated as
    unconstrained capacity (the user's toggle semantics)."""
    ...
    if best_pt is None:
        return None
    routed = graph.least_cost_to_target([lat, lon], best_pt, best_name)
    if routed is None:
        return None
    _cost, legs = routed
    transport, liq = transport_from_legs(legs)
    out = {"dest_name": best_name, "in_basin": False, "total_km": sum(l["km"] for l in legs),
           "modes": sorted({l["mode"] for l in legs}), "legs": legs}
...
def main():
    ...
    for plant in plants:
        ...
        b = basin_route(graph, plant, basins)
        if w is None:
            n_no_well += 1
        elif "pipeline" in w["modes"]:
            n_pipe_well += 1
    ...
    for nm in ("West County Energy Center", "Union Power Station", "Crystal River"):
        ...
        if w:
            print(f"  well : {w['dest_name']} [{w['confidence']}] {w['total_km']}km "
                  f"{'+'.join(w['modes'])} -> transport ${w['transport_usd']} +liq ${w['liquefaction_usd']}"
                  f" +stor ${w['storage_usd']} = ${w['total_usd']}/t (${w['annual_usd_m']}M/yr)")
        if b:
